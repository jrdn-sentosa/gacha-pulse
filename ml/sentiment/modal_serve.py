"""Modal deployment of the sentiment classifier FastAPI service (ml/serve.py).

Builds a container image containing exactly what serve.py needs to import and run:
serve.py itself, its two sibling modules -- common.py (defines MODELS_DIR, which serve.py
imports) and pipeline_def.py (the module joblib needs importable by name to unpickle the
custom ReviewStatsTransformer step inside pipeline.joblib) -- and the fitted bundle itself
(models/pipeline.joblib). Everything else in ml/ (training scripts, data/, other models/
artifacts) is deliberately left out of the image.

scikit-learn is pinned to exactly 1.9.1, matching the bundle's confirmed
metadata["sklearn_version"] (see ml/build_pipeline.py's output and ml/pyproject.toml) --
any other installed version would either reintroduce InconsistentVersionWarning or,
worse, an incompatible unpickle.
...
Deploy:  modal deploy ml/modal_serve.py
"""
...
image = (
    ...
    .pip_install(
        ...
        "joblib==1.6.0",
        "pandas==3.0.5",  # imported transitively by common.py
        "fastapi==0.141.1",
        "pydantic==2.13.5",
    )
    .add_local_file(SCRIPT_DIR / "serve.py", f"{CONTAINER_APP_DIR}/serve.py", copy=True)
    .add_local_file(SCRIPT_DIR / "common.py", f"{CONTAINER_APP_DIR}/common.py", copy=True)
...
app = modal.App("gacha-sentiment-classifier")
...
# empirically on the sibling eos_risk deployment: rapid requests all returned 200 before
# this was added). One container is a deliberate, honest tradeoff for "basic" abuse
# protection on a low-traffic internal API -- a correct limit across replicas would need
# a shared store (e.g. Redis), which is out of scope here.
